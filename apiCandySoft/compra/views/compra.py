...
class CompraViewSet(viewsets.ModelViewSet):
    ...
    def cambiar_estado(self, request, pk=None):
        ...
        try:
            ...
            # Actualizar stock solo si el estado cambia a "Completada" (asumiendo ID 3)
            if nuevo_estado.id == 3:
                self._actualizar_stock_al_completar_compra(compra)
                # Recargar el serializer para reflejar posibles cambios (aunque no es estrictamente necesario aquí)
                serializer = self.get_serializer(compra)
                response_data = serializer.data
